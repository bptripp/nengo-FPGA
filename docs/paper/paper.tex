...
\abstract{abstract goes here - bryan???}
...
- from my other paper for mind collection
...
of the system (in this case an ensemble of neurons). To decode a continuous
estimate of the input from neural activity, the NEF focuses on the
postsynpatic filtered activity generated by the reception of a spike
...
For reasons that will be apparent in a moment, we use a \textit{linear}
decoder:
...
Any optimization method can be used to find these decoders. The simplest
is to use standard least-squares optimization:
\begin{equation}
...
is minimized and the integral is over all $\mathbf{x}$ values. It
has been shown that linear decoders are sufficient to decode \textasciitilde{}95\%
of the information available in a single spike train generated by
a stimulus \cite{Rieke1997b}. Furthermore, as the number of neurons
$N$ increases, the mean squared error decreases as $1/N$. The NEF
decoding process is depicted in Figure \ref{fig:NEF-decoding}, where
the optimal linear decoders have been found and used for twenty neurons.
Temporal decoding is also linear, as described, and is performed using
the postsynaptic, current-based filters (see section \ref{sub:Principle-3--}
for further discussion).

While linear decoders are useful for visualizing the information encoded
within the activity of a group of neurons, they also provide a direct
way to solve for connection weights between groups of neurons. This
...
and then use that result to solve for the connection weights. Importantly,
given the characterization of neural activity with preferred direction
vectors, there is no difference between using this smaller space and
using the equivalent full connection matrix. Interestingly, this general
technique has started to be applied in broader domains; see \cite{tapson2013}
for an overview.

For example, if a connection between neural groups is meant to compute
...
in B. Behavior of this network is shown in Figure \ref{fig:Using-NEF-weights}a. 

While the least-squares method for optimization we use here is not
biologically plausible on its own, we have also shown that biologically
realistic learning rules will converge on a similar solution \cite{MacNeil2011a}.
These realistic rules are, however, several orders of magnitude more
computationally expensive.

\subsection{Principle 2 - Transformation}

Connections between groups of neurons can also compute functions other
than the identity function. That is instead of \textbf{$\mathbf{y}=\mathbf{x}$}
we can do \textbf{$\mathbf{y}=f(\mathbf{x})$}. We do this by finding
...
\end{equation}
Figure \ref{fig:Using-NEF-weights}b shows the computation of the
element-wise square ($f(\mathbf{x})=[x_{1}^{2},x_{2}^{2}]$).
...
of any desired function of the vector $\mathbf{x}$, the NEF also
provides a method for computing functions of the form \textbf{$\mathbf{\frac{\mathrm{d}x}{\mathrm{dt}}=}f(\mathbf{x},\mathbf{u})$},
where $\mathbf{u}$ is the input from some other population. We do
this by exploiting the fact that neurons do not simply accept input
...
ms (AMPA; \cite{jonas1993quantal}) up to \textasciitilde{}100ms (NMDA;
...
neural models that compute abitrary dynamics. Given a neural population
...
attractor networks \cite{Eliasmith2005p}. Figure \ref{fig:Two-dynamical-systems}a
shows a single neural population with a recurrent feedback connection
computing the standard linear oscillator \textbf{$\mathbf{\frac{\mathrm{d}x}{\mathrm{dt}}=}[-x_{2},-x_{1}]$}.
Here, the post-synapic time constant $\tau$ is 100ms, but the period
of oscillation is $2\pi$ (\textasciitilde{}6.28 seconds). Importantly,
we achieve different periods of oscillation without changing the neural
property $\tau$; rather, we change the function being computed by
the feedback connection, resulting in a different set of connection
weights. For example, we could compute \textbf{$\mathbf{\frac{\mathrm{d}x}{\mathrm{dt}}=}[-x_{2}/2,-x_{1}/2]$},
giving a period of $\pi$, instead. This same method works for nonlinear
functions as well; in Figure \ref{fig:Two-dynamical-systems}b we
show the classic Lorenz attractor \textbf{$\mathbf{\frac{\mathrm{d}x}{\mathrm{dt}}=}[10(x_{2}-x_{1}),x_{1}(28-x_{3})-x_{2},x_{1}x_{2}-\frac{8}{3}x_{3}]$}.
...
It should be noted that networks constructed with this approach are
fundamentally different from Echo State Networks \cite{jaeger2001}
and Liquid State Machines \cite{Maass2002}. In both of those approaches,
the recurrent connections are randomly chosen, and then a linear combination
of the outputs are found that compute the desired function. With the
NEF, we also find a linear combination of the outputs ($\mathbf{d}_{i}$),
but we solve for the ideal recurrent connection weights to achieve
the desired dynamics. This makes the NEF much more efficient and capable
of computing a much wider range of dynamics.



\subsection{Nengo}

The principles of the NEF have been incorporated into the Nengo neural simulator \cite{Bekolay2014}.  This simulator

- mention spaun

These three principles are sufficient to implement all of our neural
models. However, to simplify the process of constructing these models,
we have developed an open-source software package known as Nengo (Neural
ENGineering Objects) that creates and runs these models. Models can
be built in Nengo using a drag-and-drop graphical user interface or
specified using the Python scripting language. Full details and documentation
can be found online at http://nengo.ca, and in other publications
\cite{Stewart2009l}.

For example, to create the model shown in Figure \ref{fig:Using-NEF-weights}a,
...
For Figure \ref{fig:Using-NEF-weights}b we need to compute the element-wise
square. This is specified in a Python function as follows:
...
Nengo will automatically solve for the connection weights that will
best approximate the provided function.
...
with a running model. This interface allows the generated plots to
be exported, and was used to produce the previous figures in this
paper (i.e. all figures in section \ref{sec:The-Neural-Engineering}).
Furthermore, Nengo scales up to our largest models: the 2.5 million
neuron Spaun model is run in Nengo, and can be downloaded at http://models.nengo.ca/spaun.
